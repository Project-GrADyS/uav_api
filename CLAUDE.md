...
HTTP REST API for controlling ArduPilot-compatible UAVs — QuadCopters (stable) and ArduPlanes (beta). Supports real drones via MAVLink and simulated drones via ArduPilot SITL.
...
| `uav_api/log.py` | Logger configuration (file + console, per-component); routes `VEHICLE` token to `COPTER`/`PLANE` logger based on `--vehicle`. Also builds Hypercorn log config dict for `--udp` mode |
...
> The test suite drives **Copter only**. Plane (`--vehicle plane`) has no integration tests yet (beta).
...
| `--vehicle` | `copter` | Vehicle type — `copter` or `plane` (beta). Selects which routers register and which ArduPilot SITL spawns. |
...
- `→ .claude/docs/specification.md` — **authoritative HTTP contract** for the default (`--vehicle copter`) endpoint set. The whole GrADyS ecosystem (gradys-embedded and gradys-gs) consumes this spec. Endpoint paths, query/body schemas, response envelopes. Open when adding or changing any copter endpoint.
- `→ .claude/docs/plane-support.md` — plane (beta) status, code locations, vehicle-selection wiring, plane-only endpoint reference, and behavioural differences vs Copter. Open when touching anything plane-side or wondering whether a copter behaviour transfers.
